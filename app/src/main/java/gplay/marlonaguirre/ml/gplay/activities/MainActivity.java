...
public class MainActivity extends AppCompatActivity {
    ...
    ArrayList<String> songs_list,folders_list;
    Set<String> all_folders_list;
    TabHost tabHost;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        HashSet<String> hashSet = new HashSet<String>(folders_list);
        folders_list.clear();
    ...
    private ArrayList<String> searchSongs(File path) {
        for(File file : path.listFiles()){
            if(file.isFile()  && file.getName().endsWith(".mp3") && !file.isHidden()){
                folders_list.add(file.getParentFile().getName());
